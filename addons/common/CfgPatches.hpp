/*  CfgPatches
    ...
    essentially acts as a load order. You want your mod to load AFTER the addons it depends on. This is so that the
    dependency doesn't overwrite the mod that depends on it.

    Now that we are in the common addon, we need to put main in our list of required addons to make sure that main
    loads first
